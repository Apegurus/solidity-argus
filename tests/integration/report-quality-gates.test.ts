...
describe("report quality gates", () => {
  ...
  test("findings are sorted deterministically by severity, file, and line", async () => {
    ...
    const critPos = result.report.indexOf(
      "### [CRIT-1] Critical A 8 · severity: Critical · evidence: Medium",
    )
    const highAPos = result.report.indexOf(
      "### [HIGH-1] High A 5 · severity: High · evidence: Medium",
    )
    const highA10Pos = result.report.indexOf(
      "### [HIGH-2] High A 10 · severity: High · evidence: Medium",
    )
    const highZPos = result.report.indexOf(
      "### [HIGH-3] High Z 20 · severity: High · evidence: Medium",
    )
    const medPos = result.report.indexOf(
      "### [MED-1] Medium A 1 · severity: Medium · evidence: Medium",
    )
