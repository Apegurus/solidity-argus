      ...
      const result = await executeReportGeneration(
        {
          ...
          scope: ["Vault.sol"],
          audit_state: JSON.stringify({ sessionId, findings }),
        },
      ...
      const firstResult = await executeReportGeneration(
        {
          ...
          scope: ["Vault.sol"],
          audit_state: JSON.stringify({ sessionId: "run-alpha", findings }),
        },
        context,
        deps,
      )
      ...
      const secondResult = await executeReportGeneration(
        {
          project_name: "DiffRunTest",
          scope: ["Vault.sol"],
          audit_state: JSON.stringify({ sessionId: "run-beta", findings }),
        },
        context,
        deps,
      )
