...
import { ScvdClient, ScvdApiError, ScvdNetworkError } from "../../src/knowledge/scvd-client";
import type { ScvdFinding } from "../../src/knowledge/scvd-client";
import { syncAll, isSyncStale } from "../../src/knowledge/scvd-sync";
...
function makeFinding(
  id: string,
  severity: ScvdFinding["severity"] = "High"
): ScvdFinding {
  return {
    scvd_id: id,
    doc_id: `doc-${id}`,
...
describe("failure mode: network timeout", () => {
  test(
    "network timeout triggers retries then returns error result",
    async () => {
...
describe("failure mode: partial response", () => {
  ...
  test("undefined payload from fetchAllFindings classified as parse error", async () => {
    ...
    client.fetchAllFindings = (async () =>
      undefined) as unknown as typeof client.fetchAllFindings;
