...
- Added provider-neutral per-agent `variant` overrides with role-specific defaults: Argus `max`, Sentinel `high`, Pythia `high`, Audit Specialist `xhigh`, Scribe `high`, and Themis `xhigh`.
