...
- **NEXT = #16 (§5 step 5) record_finding reject-before-mutate (tool-tracking-hook:449, I6).** *Direction (needs tracing):* the record-finding handler calls `store.addFinding(findingPayload)` at :449 — a live-state mutation. I6 requires: validate → canonicalize → append `finding.added` to the durable journal (fail-fast) → **then** mutate; on no durable sink (the #15 UNACTIVATED/DEGRADED state) **reject before mutating** (never an optimistic append). Trace where/if the journal append currently happens relative to `store.addFinding`, and how the sink is obtained in this handler (is a sink even in scope here, or does the caller append events after this returns a count?). Reconcile with #15's "no durable sink" state.
- **THEN, in §5 order:** #19 persist report metadata (run-finalizer:101) → #18 seal-only-on-success/emit `run.finalization_failed` (run-finalizer:469) → #14+#13 recovered-identity + reportGenerated-non-terminal (session-activation:167/:153) → #17 orphan-buffer caps+clear (tool-tracking-hook:658) → #20 activatedSessions guard before archive (create-hooks:415). Then WS-5 #25/#26/#27.
