...
- **✅ FOUNDATION (§5 step 1) — DONE, pushed `edd608e`** (1940 pass, tsc+biome clean): `EventSink` gains explicit state (`ACTIVE|DRAINING|SEALED|FAILED_RECOVERABLE`) + `ownerSet` (I1/I11); new `run.finalization_failed` journal event → sink FAILED_RECOVERABLE **without** sealing (unblocks #18/I3); `bounded-sink-registry` maintains `ownerSet` (add on setForSession / remove on deleteSession). Additive: `isFinalized` is a derived getter (SEALED-only) so all 19 consumers + mocks unchanged. **Ordering refinement:** the `SCHEMA_VERSION` bump is moved to WS-5 #27 (rides its copy-on-read migration) so 2.0.0 journals stay readable in-between.
- **NEXT, in §5 order:** #15 sink-init-failure→not-activated (session-activation:196 — *fix designed: set `sessionActivated` only after sink+first-append succeed, roll back partial sink refs in catch, drop the unconditional flag at :245; test must force the real `createEventSink` to fail*) → #11 referenced-exempt eviction (bounded-sink-registry:45, reads the new ownerSet) → #12 async flushAndDispose (session-state-registry:26) → #16 record_finding reject-before-mutate (tool-tracking-hook:449) → #19 persist report metadata (run-finalizer:101) → #18 seal-only-on-success/emit `run.finalization_failed` (run-finalizer:469) → #14+#13 recovered-identity + reportGenerated-non-terminal (session-activation:167/:153) → #17 orphan-buffer caps+clear (tool-tracking-hook:658) → #20 activatedSessions guard before archive (create-hooks:415).
