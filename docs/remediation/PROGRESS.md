...
- **NOW = WS-5 (§5 remainder):** #25 content-dedup on hydration (finding-store:55) → #26 `hasFinding` path normalization (:92) → #27 schema copy-on-read migration + typed `MigrationError` + `SCHEMA_VERSION` bump (schemas:458).
