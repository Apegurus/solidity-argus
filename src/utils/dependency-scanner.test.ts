...
describe("scanDependencyRisks", () => {
  test("flags outdated OpenZeppelin < 4.9 as high risk", () => {
    ...
    const ozRisk = risks.find((r) => r.package === "@openzeppelin/contracts" && r.risk === "high")
  ...
  test("flags upgradeable contracts without hardhat-upgrades as medium risk", () => {
    ...
    const upgradeRisk = risks.find(
      (r) => r.package === "@openzeppelin/contracts-upgradeable" && r.risk === "medium",
    )
  ...
  test("detects multiple risks simultaneously", () => {
    ...
    const ozHigh = risks.find((r) => r.package === "@openzeppelin/contracts" && r.risk === "high")
    const upgradeMedium = risks.find(
      (r) =>
        r.package === "@openzeppelin/contracts-upgradeable" && r.category === "missing-tooling",
    )
    const solmateMedium = risks.find((r) => r.package === "solmate" && r.risk === "medium")
  ...
  test("flags outdated solmate < 6.0 as medium risk", () => {
    ...
    const solmateRisk = risks.find((r) => r.package === "solmate" && r.risk === "medium")
  ...
  test("flags OZ < 5.0 as low risk for upgrade suggestion", () => {
    ...
    const ozLow = risks.find((r) => r.package === "@openzeppelin/contracts" && r.risk === "low")
  ...
  test("handles caret and tilde version ranges correctly", () => {
    ...
    expect(
      caretRisks.some((r) => r.package === "@openzeppelin/contracts" && r.risk === "high"),
    ).toBe(true)
    ...
    expect(
      tildeRisks.some((r) => r.package === "@openzeppelin/contracts" && r.risk === "high"),
    ).toBe(true)
  })
})
