...
test("detects *audit*.md files as audit-report artifact", () => {
  ...
  expect(artifacts.some((a) => a.type === "audit-report" && a.name === "security-audit.md")).toBe(
    true,
  )
  expect(artifacts.some((a) => a.type === "audit-report" && a.name === "audit-findings.md")).toBe(
    true,
  )
})
...
test("detects slither-report* files as slither-output artifact", () => {
  ...
  expect(artifacts.length).toBe(2)
  expect(artifacts.some((a) => a.type === "slither-output" && a.name === "slither-report.md")).toBe(
    true,
  )
  expect(
    artifacts.some((a) => a.type === "slither-output" && a.name === "slither-report-2024.json"),
  ).toBe(true)
})
...
test("detects mythril-report* files as security-tool-output artifact", () => {
  ...
  expect(artifacts.length).toBe(2)
  expect(
    artifacts.some((a) => a.type === "security-tool-output" && a.name === "mythril-report.md"),
  ).toBe(true)
  expect(
    artifacts.some(
      (a) => a.type === "security-tool-output" && a.name === "mythril-report-2024.json",
    ),
  ).toBe(true)
})
...
test("detects securify-report* files as security-tool-output artifact", () => {
  ...
  expect(artifacts.length).toBe(2)
  expect(
    artifacts.some((a) => a.type === "security-tool-output" && a.name === "securify-report.md"),
  ).toBe(true)
  expect(
    artifacts.some(
      (a) => a.type === "security-tool-output" && a.name === "securify-report-2024.json",
    ),
  ).toBe(true)
})
...
test("detects multiple artifact types in same directory", () => {
  ...
  expect(
    artifacts.some((a) => a.type === "deployment-artifact" && a.name === ".openzeppelin"),
  ).toBe(true)
  expect(artifacts.some((a) => a.type === "audit-report" && a.name === "security-audit.md")).toBe(
    true,
  )
