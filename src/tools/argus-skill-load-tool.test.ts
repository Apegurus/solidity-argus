...
describe("argusSkillLoadTool", () => {
    ...
    const content = await executeArgusSkillLoad(
      ...
      createContext(),
      {
        loadConfig: () => ({
          ...
          tools: {},
           knowledge: { scvd: { enabled: true, apiUrl: "https://api.scvd.dev" }, autoSync: true, skillPrecedence: "bundled-first" as const },
    ...
    const content = await executeArgusSkillLoad(
      ...
      createContext(),
      {
        loadConfig: () => ({
          ...
          tools: {},
          knowledge: { scvd: { enabled: true, apiUrl: "https://api.scvd.dev" }, autoSync: true, skillPrecedence: "bundled-first" as const },
          reporting: { format: "markdown" as const, severityThreshold: "low" as const, gasAnalysis: false },
          solodit: { enabled: true, port: 3000 },
          disabled_hooks: [],
          hooks: {},
          cli: {},
  ...
  it("includes provenance section when source_url and source_license are present", async () => {
    ...
    expect(content).toContain("[Provenance: MIT | https://github.com/kadenzipfel/smart-contract-vulnerabilities | Imported: 2025-01-15T00:00:00Z]")
  })
